 ...
 * @LastEditTime: 2025-10-29 10:06:15
 * @Description:
 */

import { DEVICE_POINT_DATA_WS_TOPIC_TEST } from "@configs/mqtt-info.ts"
...
import { getStateForType } from "@/pages/area-state-overview-v2/methods/test"
...
function getDvsWsTopicMap(deviceTypeList: IParams["deviceTypeList"]) {
  if (!deviceTypeList?.length) return DEVICE_POINT_DATA_WS_TOPIC_TEST
  return deviceTypeList.reduce(
    (prev, next) => {
      prev[next] = DEVICE_POINT_DATA_WS_TOPIC_TEST[next]
      return prev
    },
    {} as typeof DEVICE_POINT_DATA_WS_TOPIC_TEST,
  )
}
