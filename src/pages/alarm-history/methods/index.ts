 ...
 * @LastEditTime: 2025-01-13 11:17:02
 * @Description:
 */
  ...
  if (res?.list) {
    VirtaulList = res.list.map((i) => {
      return {
        ...
        systemName: sysList.length ? sysList.find((j) => j.value === i.systemId)?.label || i.systemId : i.systemId,
        alarmLevelName: alarmLevelList.length
          ? alarmLevelList.find((j) => j.value === i.alarmLevelId)?.label || i.alarmLevelId
          : i.alarmLevelId,
        brakeLevelName: brakeLevelList.length
          ? brakeLevelList.find((j) => j.value === i.brakeLevelId)?.label || i.brakeLevelId
          : i.brakeLevelId,
      }
    })
  }
