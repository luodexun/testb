 ...
 * @LastEditTime: 2025-10-11 10:03:24
 * @Description:
 */
import { doBaseServer } from "@/api/serve-funs"
import { StorageDeviceStdNewState } from "@/configs/storage-cfg"
...
import { STATE_TYPE } from "./test"

// 根据设备类型过滤出状态列表，并根据SUB类型下parentId推断出属于哪些MAIN类型的子数据
export const getStateList = async (deviceType) => {
  const dvsStateList = getStorage<Array<IModelListData>>(StorageDeviceStdNewState)
  let actualDvTyState: Array<IModelListData> = []
  const mainState = dvsStateList?.filter((i) => i.deviceType === deviceType && i.stateType === "MAIN")
  const subState = dvsStateList?.filter((i) => i.deviceType === deviceType && i.stateType === "SUB")
...
//   })
// }
export async function getTypeDeviceSignRecordData(params: {
  deviceType?: string
  stationId?: number
  isEnd?: boolean
}) {
...
export const getDvsState = async (deviceType: TDeviceType) => {
  const mainState = deviceType === "PVINV" || deviceType === "ESPCS" ? "5,6" : "6,7"
  const params = {
    mainState,
    firstFlag: "1",
  }
