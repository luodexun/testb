...
export default function StateOverviwe(props) {
  ...
  const currntStationList = useMemo(() => {
    ...
    return getTypeStationList(activeKey, "id", false)
  ...
  useEffect(() => {
    ...
    let result = {}
    // Promise.all(currntStationList.map((item) => getDeviceData(item.value, activeKey))).then((res) => {
    //   result = res.reduce((prev, cur) => {
