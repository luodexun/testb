...
const layoutSizeTrans = (name) => {
  let size = ""
  size = name === "华中" || name === "四川" ? "240%" : "90%"
  return size
}
const sizeTrans = (data, screenWidth) => {
  if (data.type === "PVINV") {
    return [33 * (screenWidth / 4513), 23 * (screenWidth / 4513)]
  } else if (data.type === "ESPCS") {
    return [33 * (screenWidth / 4513), 20 * (screenWidth / 4513)]
...
const getMapName = (name) => {
  if (!name) return "华中"
  return name === "华中" ? "华中框" : name
}
export function geoOption(params: geoChartData) {
  const { series, screenWidth, mapName, large } = params || { series: [], large: false, mapName: "华中" }
    ...
    geo: [
      ...
      },
      {
        ...
        itemStyle: {
          ...
          borderColor: mapName === "华中" ? "rgba(207, 228, 245, 0.27)" : "#c0f3fb",
          borderWidth: 1,
          borderType: mapName === "华中" ? "dashed" : "solid",
          shadowColor: "#8cd3ef",
          // shadowOffsetY: 10,
