...
export interface IControlBatchTreeProps {
  ...
  onSubmit: (checkedDevices: IBatchStn2DvsTreeData[], isSubmit: boolean, type) => void
  searchDevice?: string
    onTreeDataChange?: (treeData: IBatchStn2DvsTreeData[]) => void // 新增回调 prop

}
export default function ControlBatchTree(props: IControlBatchTreeProps) {
  const { deviceType, onSubmit, searchDevice, onTreeDataChange } = props
  //树结构数据
  ...
  useEffect(() => {
    if (onTreeDataChange) {
      onTreeDataChange(device4Tree);
    }
  }, [device4Tree, onTreeDataChange]);

  // 修改获取树数据的 effect
  useEffect(() => {
    setCheckedKeys([])
    if (treeData4DvsTypeMapRef.current[deviceType]?.length) {
      const treeData = treeData4DvsTypeMapRef.current[deviceType];
  ...
  const treeCheckRef = useRef<TreeProps["onCheck"]>((_chKeys, info) => {
    ...
    let checkedStn = []
    let checkedDvs = []
    checkedNodes.length && checkedNodes.forEach((item:any) => {
      if(!item.isLeaf && !item.key.startsWith("1_")) {
        checkedStn.push(item.stationCode)
      }
    })
    checkedNodes.length && checkedNodes.forEach(item => {
      if(item.isLeaf) {
        if(!checkedStn.includes(item.stationCode)) {
          checkedDvs.push(item.deviceCode)
        }
      }
    })
    if(checkedDvs.length) {
      onSubmit(dvsList, false, 'dvs')
    } else if(checkedStn.length) {
      onSubmit([...new Set([...checkedStn])], false, 'stn')
    } else {
      onSubmit(dvsList, false, 'dvs')
    } 
  })
