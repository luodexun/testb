 ...
 * @LastEditTime: 2024-08-15 17:21:27
...
export function LineOrBarOption(params: lineChartData) {
  const { xAxis, series, screenWidth, unit, axisLabel, yAxisTwoUnit } = params || { xAxis: [], series: [] }
  const actTooltip = {
    ...baseTooltip,
    formatter: function (params) {
      let res = `<div style="font-size: 12px;line-height: 1.2em">${params[0].name}</div>`
      for (let i = 0; i < params.length; i++) {
        res += `<div style="font-size: 12px;line-height: 1.2em">${params[i].marker} ${params[i].seriesName} : <span style="color: #00F7FA">${parseNum(params[i].data)}</span> ${params[i].seriesIndex === 1 ? "h" : unit} </div>`
      }
      return res
    },
  }
  const option: EChartsOption = {
    grid: { left: "5%", right: "3%", top: "50", bottom: "7%", containLabel: true },
