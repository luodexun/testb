 ...
 * @LastEditTime: 2024-07-29 11:36:47
 * @Description:
 */
...
const QuotaForm = forwardRef((props, ref) => {
  ...
  const addCur = () => {
    setQuotaList((prev) => {
      const newItem = [{ id: Date.now(), forecastTime: "", shortPredProduction: 0 }]
      ...
      {quotaList?.map((i) => {
        return (
          <div key={i.id} className="reset-item">
            ...
            <InputNumber
              defaultValue={i.shortPredProduction}
              value={i.shortPredProduction}
              style={{ width: "38%" }}
              onChange={(e) => onChange(e, "shortPredProduction", i.id)}
            />
