...
const SXSiteForm = forwardRef((props, ref) => {
  ...
  const addSite = () => {
    ...
    //   }, {})
    setSiteList((prev) => {
      const newItem = [{ id: Date.now(), name: "", value: "", stnDeviceType: "", tooltipContent: { ...initCt } }]
        ...
        {siteList.map((i, index) => {
          return (
            <div key={i.id} className="reset-item">
              ...
              /> */}
              <Select
                defaultValue={i.stnDeviceType}
                value={i.stnDeviceType}
                style={{ width: "100px" }}
                allowClear
                options={[
                  { value: "WT", label: "风机" },
                  { value: "PVINV", label: "光伏" },
                  { value: "Control", label: "集控中心" },
                ]}
                onChange={(e) => onChangeSite(e, "stnDeviceType", index)}
              />
              <Input
